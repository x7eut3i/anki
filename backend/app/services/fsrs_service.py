...
class FSRSService:
    ...
    def review_card(
        self, card_data: dict, rating: int, review_time: datetime | None = None
    ) -> tuple[dict, dict]:
        """
        Review a card and return updated card data + review log.

        Args:
            card_data: Dict with FSRS fields from DB card
        ...
        pre_state = card_data.get("state", 1)
    ...
    def preview_ratings(
        ...
        for rating_int, rating_enum in [
            ...
            preview_card = FSRSCard.from_json(fsrs_card.to_json())
    ...
    def _dict_to_fsrs_card(self, data: dict) -> FSRSCard:
        ...
        # FSRS: Learning=1, Review=2, Relearning=3
        state_val = data.get("state", 0)
        if isinstance(state_val, int):
            if state_val == 0:
                # New card — treated as Learning in FSRS
