...
def make_poem_expand_user_prompt(poem_names: list[str]) -> str:
    ...
    return (
        ...
        f"诗词名列表：{poems_str}\n\n"
        f"═══ 生成规则 ═══\n"
        f"1. 对每首诗/词/古文，找出其中所有名句（著名的、常考的、有文学价值的句联），每个名句生成一张填空卡片\n"
        f"2. 卡片数量由作品长度和知名度决定，不设上限：\n"
        f"   - 绝句（如《静夜思》）：2-4 张\n"
        f"   - 律诗/词（如《登高》《念奴娇》）：4-8 张\n"
        f"   - 长诗/长词（如《琵琶行》《长恨歌》）：10-20 张\n"
        f"   - 名篇古文（如《滕王阁序》《出师表》《岳阳楼记》）：15-30 张甚至更多\n"
...
3. 【自包含原则】所有字段（front、back、explanation、distractors、alternate_questions、facts、knowledge中的所有文本）必须完全独立、自包含：
   ...
   c) 禁止依赖选项作为上下文（front和alternate_questions的question必须能作为问答题独立使用），严禁出现"下列""以下""哪个""哪种"等指代性词语：
...
7. 拼音与注音规则（极其重要）：
   a) 生僻字和易读错字必须行内注音（用括号标注拼音），确保读者能正确朗读。判断标准：
      - 生僻字（现代汉语中不常用、普通高中生不一定认识的字）→ 必须注音
      - 多音字（在此语境下读音容易读错）→ 必须注音
      - 古文/诗词中的冷僻用字 → 必须注音
      ✅ 需要注音举例：龃龉(jǔ yǔ)、踟蹰(chí chú)、觊觎(jì yú)、桎梏(zhì gù)、掣肘(chè zhǒu)、赓续(gēng xù)、踔厉(chuō lì)、廿(niàn)、圩(xū)、闾(lǘ)、舸(gě)、舳(zhú)、骖(cān)、騑(fēi)、睢(suī)、翾(xuān)、彀(gòu)
      ❌ 不需要注音：常用词（力戒、形式主义、杜绝、官僚主义、人民群众等）一律不加注音
   b) 成语卡片额外要求：必须在meta_info.pinyin字段标注完整拼音（如"zhāng guān lǐ dài"）。成语的back字段只放释义，绝不放拼音
   c) 原则：宁多注不漏注——如果你不确定读者是否认识这个字，就加注音
8. 【卡片独立性原则】每张卡片必须完全独立，禁止在任何字段中引用其他卡片的内容：
   ❌ 禁止写法："同上" "见前文" "见《滕王阁序》全篇" "（全诗同上）" "参见第X张卡片" "与上题相同"
   ✅ 正确写法：每张卡片的每个字段（包括facts.full_poem等）都必须填写完整内容，即使多张卡片出自同一首诗/同一篇文章，每张卡片也要独立写完整
9. 每张卡片的meta_info.alternate_questions必须至少包含2个变体题（不同角度考察同一知识点的选择题），每个变体题必须有3个distractors（错误选项）和正确answer，系统运行时会自动将answer和distractors组合并打乱为4个选项
"""
...
def make_pipeline_user_prompt(
    ...
    return (
        ...
        f"以下类型必须逐一提取，有多少出多少，严禁遗漏任何一个：\n"
        f"  - 成语：逐字逐句扫描全文，文章中出现的【每一个】成语都必须生成一张卡片。\n"
        f"    包括但不限于四字成语、常见惯用语。哪怕文章只是顺带提及也要抓出来。\n"
        f"    ⚠️ 常见遗漏：嵌入在长句中的成语、引用段落里的成语、标题里的成语\n"
        f"  - 古诗词/古文名句：每句/每联都值得出题\n"
        f"  - 习近平金句/重要论述/领导人讲话：每条金句都应生成填空卡片。\n"
        f"    金句识别标准：带引号的领导人原话、政策文件中的精炼表述、排比句式、对仗工整的总结性语句\n"
        f"  - 规范表述/公文用语：文章中出现的规范公文表述都应生成卡片（口语↔公文对照）\n"
...
⚠️ 重要：你必须直接输出纯JSON对象，禁止输出任何非JSON内容（不要有开头说明、不要有markdown代码块标记如```json、不要有结尾总结）。
⚠️ 如果原文中包含HTML残留标记、乱码、广告片段或网页导航等噪音内容，请自动忽略这些部分，只分析正文实质内容。
  ...
  "highlights": [
    {
      ...
      "annotation": "精要批注（1-2句话点明重要性和考试价值）"
    }
  ],
  "overall_analysis": {
    "theme": "文章主题",
    ...
    "possible_questions": [
      {
        ...
        "reference_answer": "参考答案（200-300字），按照该题型的标准答题格式撰写"
      }
    ]
  },
  "vocabulary": [
...
3. highlights数量必须充足——这是精读的核心价值，宁多勿少！按文章长度严格执行：
   - 500字以下：至少8-10个
   - 500-1000字：至少12-18个
   - 1000-2000字：至少20-30个
   - 2000-3000字：至少30-40个
   - 3000字以上：至少40-50个，每500字应标注5-8处
   ⚠️ 每个自然段至少应有1-2处标注。如果某段没有标注，请回头检查是否遗漏了重要信息、政策术语、数据或金句
   ⚠️ 特别注意文章中后部分的内容，不要只标注开头几段就草草结束
4. annotation（批注）要精炼有价值：
   - 每条annotation用1-2句话点明重要性、考试价值或应用场景
   - 遇到"四个意识""四个自信""两个维护"等政治术语，必须在annotation中列出具体内容（如"政治意识、大局意识、核心意识、看齐意识"）
   - 遇到专有名词（如"新质生产力""双碳目标"等），必须在annotation中简要解释含义
   - 遇到规范用语，给出口语对照
   - 遇到数据，补充关键背景
...
9. exam_points.possible_questions 只需提供最有价值的1个考题，附上参考答案（reference_answer），按对应题型标准答题格式撰写
10. exam_points 要具体实用，不要泛泛而谈
11. 严格只输出JSON对象，第一个字符必须是{，最后一个字符必须是}，禁止任何开头说明、markdown标记或结尾总结
"""
...
def make_article_analysis_prompt(title: str, content: str) -> str:
    ...
    return (
        f"请对以下文章进行深度精读分析。\n\n"
        f"文章标题：{title}\n"
        f"文章字数：约{word_count}字（请根据字数确保highlights数量充足）\n\n"
        f"文章全文：\n{content[:]}\n\n"
        f"请严格按照system prompt中的JSON schema返回精读报告，直接输出JSON，不要包含任何其他内容。"
    )
